...
import {
  AppBar, Toolbar, Typography, Button, Box, IconButton, Menu, MenuItem, Container, 
  Drawer, List, ListItem, ListItemIcon, ListItemText, Divider, Avatar
} from '@mui/material';
...
const Navbar = () => {
  ...
  // --- ESTADOS DEL MODAL DE LOGIN/REGISTRO ---
  const [authModalOpen, setAuthModalOpen] = useState(false);
  const [initialTab, setInitialTab] = useState(0); // 0=Login, 1=Registro
  ...
  const navLinkStyle = {
    color: '#e0e0e0', fontWeight: 500, textTransform: 'none', fontSize: '0.95rem', mx: 1, '&:hover': { color: '#ffffff' }
  };
    ...
    <Box onClick={handleDrawerToggle} sx={{ textAlign: 'center' }}>
      ...
      <List>
        ...
        {estaLogueado ? (
          ...
          <>
            {/* EN MÓVIL TAMBIÉN USAMOS EL MODAL */}
    ...
    <>
            ...
            <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 2, alignItems: 'center' }}>
              {estaLogueado ? (
                ...
                <>
                  ...
                  <Button onClick={openLogin} sx={{ color: 'white', fontWeight: 600 }}>
                    Ingresar
                  </Button>
                  <Button onClick={openRegister} variant="contained" sx={{ backgroundColor: '#aeea00', color: '#000', fontWeight: 'bold', borderRadius: '8px', textTransform: 'none', boxShadow: 'none', '&:hover': { backgroundColor: '#c6ff00' } }}>
                    Registrarse
                  </Button>
                </>
              )}
      ...
      <Menu anchorEl={anchorEl} open={isMenuOpen} onClose={handleMenuClose} PaperProps={{ elevation: 4, sx: { mt: 1.5, borderRadius: '12px', minWidth: '200px' } }}>
        ...
        <MenuItem component={RouterLink} to="/perfil" onClick={handleMenuClose} sx={{ my: 0.5 }}>
          <AccountCircleIcon sx={{ fontSize: 20, mr: 1.5, color: '#666' }} /> Mi Perfil
        </MenuItem>
